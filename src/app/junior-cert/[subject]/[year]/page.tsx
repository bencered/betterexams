...
export default async function JuniorCertYearPage({ params }: { params: Promise<{ subject: string; year: string }> }) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 3,
        name: year,
        item: `https://betterexams.ie/junior-cert/${subject}/${year}`
      }
    ]
  }
  ...
  const learningResourceSchema = {
    ...
    name: `Junior Cert ${subjectInfo.name} ${year} Exam Papers`,
    description: `Past exam papers and marking schemes for Junior Cert ${subjectInfo.name} ${year}`,
