...
type ExamData = {
  lc: Record<string, Record<string, YearData>>
...
export async function generateMetadata({ params }: { params: Promise<{ subject: string; year: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://betterexams.ie/leaving-cert/${subject}/${year}`,
    },
    alternates: {
      canonical: `https://betterexams.ie/leaving-cert/${subject}/${year}`,
    },
  }
}

export default async function LeavingCertYearPage({ params }: { params: Promise<{ subject: string; year: string }> }) {
  const { subject, year } = await params
  const subjectInfo = getSubjectBySlug(subject)
  ...
  const subjectData = examData.lc[subjectInfo.num]
