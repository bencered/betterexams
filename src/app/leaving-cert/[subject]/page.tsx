...
type ExamData = {
  lc: Record<string, Record<string, { exampapers?: Array<{ details: string; url: string }>; markingschemes?: Array<{ details: string; url: string }> }>>
...
export async function generateStaticParams() {
  const subjects = Object.keys(examData.lc)
...
export async function generateMetadata({ params }: { params: Promise<{ subject: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      url: `https://betterexams.ie/leaving-cert/${subject}`,
    },
    alternates: {
      canonical: `https://betterexams.ie/leaving-cert/${subject}`,
    },
  }
}

export default async function LeavingCertSubjectPage({ params }: { params: Promise<{ subject: string }> }) {
  const { subject } = await params
  const subjectInfo = getSubjectBySlug(subject)
  ...
  const years = examData.lc[subjectInfo.num]
  if (!years) {
    notFound()
  }
  ...
  const courseSchema = {
    ...
    "@type": "Course",
    name: `Leaving Cert ${subjectInfo.name}`,
    description: `Past exam papers and marking schemes for Leaving Cert ${subjectInfo.name}`,
    provider: {
      "@type": "Organization",
      name: "Better Exams",
  ...
  return (
    <>
      ...
      {/* Prominent CTA */}
      <Link 
        href={`/?exam=lc&subject=${subjectInfo.num}`}
      ...
      <div className="mt-10 text-center">
        <h2 className="text-3xl md:text-4xl font-bold">Leaving Cert {subjectInfo.name}</h2>
      ...
      <div className="mt-8 w-full max-w-2xl space-y-4">
        {sortedYears.map((year) => {
          ...
          return (
            <Link
              key={year}
              href={`/leaving-cert/${subject}/${year}`}
